...
Optimized: detailed root cause, stricter Process Issue flag,
           distribution-form insights with %, formatted Excel output.
"""
...
# 4. INSIGHTS PROMPTS
# ─────────────────────────────────────────────
LOB_INSIGHTS_PROMPT = """You are summarizing repeat contact findings for LOB: {lob}.
Total contacts analyzed: {total}
Flag distribution: {flag_stats}

Individual findings:
{findings}

Write a structured summary with EXACTLY these 6 sections.
Each section must have 4-5 bullet points. Include counts and percentages where possible.
Use crisp, non-redundant language grounded in the data.
...
OVERALL_INSIGHTS_PROMPT = """You are summarizing repeat contact findings ACROSS ALL LOBs.
Total contacts: {total} | LOBs: {lobs}
Flag distribution: {flag_stats}

Individual findings:
{findings}

Write a structured executive summary with EXACTLY these 6 sections.
Each section must have 4-5 bullet points with counts and percentages.
Highlight cross-LOB patterns and note where LOBs differ significantly.

## 1. Top Reasons for Repeat Contacts (Overall)
## 2. Issue Resolution Patterns (Overall)
## 3. Provider Dissatisfaction Drivers (Overall)
## 4. Process Issues Distribution (Overall)
## 5. Technology Issues Distribution (Overall)
## 6. Agent Knowledge Gaps Distribution (Overall)
"""
...
# ─────────────────────────────────────────────
# 6. DATA AGGREGATION  (smart dedup + merge)
# ─────────────────────────────────────────────
...
def analyze_repeat_contact(row: pd.Series) -> dict:
    ...
    try:
        ...
        # Normalise all Yes/No flag fields — clean up any accidental "Yes: reason" merges
        for flag, reason_key in FLAG_FIELDS:
            ...
            if ":" in raw_val:
                ...
                # Only overwrite reason if the model didn't already provide a separate key
...
def _flag_stats(df: pd.DataFrame) -> dict:
    ...
    for flag, _ in FLAG_FIELDS:
        if flag in df.columns:
            ...
            pct = round(yes / len(df) * 100, 1)
            stats[flag] = f"{yes}/{len(df)} ({pct}%)"
    return stats

# ─────────────────────────────────────────────
# 9. INSIGHT GENERATORS
# ─────────────────────────────────────────────
def generate_lob_narrative(lob: str, lob_df: pd.DataFrame) -> str:
    findings = lob_df[[c for c in INSIGHT_COLS if c in lob_df.columns]].to_dict(orient="records")
    prompt = LOB_INSIGHTS_PROMPT.format(
        lob=lob, total=len(lob_df),
        flag_stats=json.dumps(_flag_stats(lob_df), indent=2),
        findings=json.dumps(findings, indent=2),
    )
    return llm.invoke([HumanMessage(content=prompt)]).content

def generate_overall_insights(final_df: pd.DataFrame) -> str:
    findings = final_df[[c for c in INSIGHT_COLS if c in final_df.columns]].to_dict(orient="records")
    lobs = final_df["lob"].unique().tolist() if "lob" in final_df.columns else []
    prompt = OVERALL_INSIGHTS_PROMPT.format(
        total=len(final_df), lobs=", ".join(lobs),
        flag_stats=json.dumps(_flag_stats(final_df), indent=2),
        findings=json.dumps(findings, indent=2),
    )
    return llm.invoke([HumanMessage(content=prompt)]).content
...
def write_excel(final_df: pd.DataFrame, narrative_df: pd.DataFrame,
    ...
    with pd.ExcelWriter(output_file, engine="xlsxwriter",
        ...
        section_fmt = wb.add_format({
            "bold": True, "bg_color": "#D6E4F0", "font_color": "#1A3A5C",
            "font_size": 11, "border": 1, "text_wrap": True, "valign": "vcenter",
        })
        lob_label_fmt = wb.add_format({
        ...
        # ── SHEET 2: LOB Insights ─────────────────────────────────────────
        ws2 = wb.add_worksheet("LOB_Insights")
        ws2.set_column(0, 0, 16)
        ws2.set_column(1, 1, 115)

        ws2.write(0, 0, "LOB", hdr_fmt)
        ws2.write(0, 1, "Insights — Distribution Form", hdr_fmt)
        ws2.set_row(0, 28)

        r = 1
        for _, row in narrative_df.iterrows():
            lob_val   = str(row.get("LOB", ""))
            ...
            for line in narrative.split("\n"):
                ...
                if not line:
                    continue
                if line.startswith("##"):
                    ws2.write(r, 0, "", wrap_fmt)
                    ws2.write(r, 1, line.lstrip("#").strip(), section_fmt)
                    ws2.set_row(r, 22)
                else:
                    ws2.write(r, 0, "", wrap_fmt)
                    ws2.write(r, 1, line, body_fmt)
                    ws2.set_row(r, 48)
                r += 1

            # Merge LOB label across block
            if r - block_start > 1:
                ws2.merge_range(block_start, 0, r - 1, 0, lob_val, lob_label_fmt)
            else:
                ws2.write(block_start, 0, lob_val, lob_label_fmt)

            # Blank separator row between LOBs
            ws2.set_row(r, 10)
            r += 1

        # ── SHEET 3: Overall Insights ─────────────────────────────────────
        ws3 = wb.add_worksheet("Overall_Insights")
        ws3.set_column(0, 0, 130)
        ws3.write(0, 0, "Overall Repeat Contact Insights — All LOBs", title_fmt)
        ws3.set_row(0, 38)
        ...
        for line in overall_text.split("\n"):
            ...
            if not line:
                continue
            if line.startswith("##"):
                ws3.write(r, 0, line.lstrip("#").strip(), section_fmt)
                ws3.set_row(r, 24)
            else:
                ws3.write(r, 0, line, body_fmt)
                ws3.set_row(r, 50)
            r += 1
...
def run_analysis(input_file: str = INPUT_FILE, output_file: str = OUTPUT_FILE):
    ...
    # Ensure all expected columns exist even on parse errors
    for col in [k for _, k in FLAG_FIELDS] + ["root_cause_summary", "Key differences"]:
        if col not in results_df.columns:
            results_df[col] = ""

    # Merge agg data + results
    ...
    # ── Insights ──────────────────────────────────────────────────────────
    print("\n📝 Generating LOB insights...")
    lob_rows = []
    for lob in final_df["lob"].unique():
        narrative = generate_lob_narrative(lob, final_df[final_df["lob"] == lob])
        lob_rows.append({"LOB": lob, "Narrative": narrative})
        print(f"  ✔ {lob}")

    print("\n📝 Generating overall insights...")
    overall_text = generate_overall_insights(final_df)

    write_excel(final_df, pd.DataFrame(lob_rows), overall_text, output_file)
    return final_df
